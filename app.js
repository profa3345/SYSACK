...
function startFirestoreListeners() {
  ...
  db.collection('ativos').onSnapshot(function(snap) {
    ...
    // Debounce dashboard update — ativos pode ter muitas mudanças simultâneas
    _debounce('ativos-dashboard', function() {
      if (isPageActive('dashboard') || isPageActive('exec-dashboard')) renderDashboard();
    }, 500);
  ...
  db.collection('switches').onSnapshot(function(snap) {
    ...
    STATE.ativos = (STATE._assetsDisc||[]).concat(STATE._assetsSw||[]);
    nbUpdate('nb-ativos', STATE.ativos.length);
    _debounce('sw-dashboard', function() {
      if (isPageActive('dashboard') || isPageActive('exec-dashboard')) renderDashboard();
    }, 500);
  ...
  db.collection('empregados').orderBy('nome').limit(2000).onSnapshot(function(snap) {
    ...
    STATE.empregadosSyncAt = primeiro?.syncAt ? new Date(primeiro.syncAt)
      ...
      : null;
    // Debounce: evita re-render em cascata quando o agente AD sincroniza em lote
...
// ── DEBOUNCE — previne re-renders em cascata ─────────────────
const _debouncers = {};
