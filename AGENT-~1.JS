...
async function registrarHistoricoLogin(dados, usuarioLogado, nowIso) {
  ...
  const hostname    = AGENT_ID.toLowerCase(); // minúsculas — painel busca com toLowerCase()
  const hostnameRaw = AGENT_ID;               // original mantido para referência
  ...
  try {
    ...
    if (!jaExiste) {
      ...
      await firestoreSet(docPath, {
        hostname, agentId: hostname, hostnameOriginal: hostnameRaw,
        usuario: usuarioNorm, usuarioNorm,
        dia, mes,
