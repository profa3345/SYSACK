...
function getUptime() {
  // os.uptime() é confiável no Windows Desktop mas pode ser impreciso
  // em Windows Server rodando como SYSTEM. Usa wmic como fallback.
  if (process.platform === 'win32') {
    try {
      ...
      if (match) {
        const raw = match[1]; // formato: YYYYMMDDHHmmss
        const boot = new Date(
          Number(raw.slice(0,4)),   // year
          Number(raw.slice(4,6))-1, // month (0-based)
          Number(raw.slice(6,8)),   // day
          Number(raw.slice(8,10)),  // hour
          Number(raw.slice(10,12)), // minute
          Number(raw.slice(12,14))  // second
        );
        const uptimeSec = Math.max(0, Math.round((Date.now() - boot.getTime()) / 1000));
        if (uptimeSec > 0 && uptimeSec < 365 * 24 * 3600) return uptimeSec;
      }
    } catch {}
  }
